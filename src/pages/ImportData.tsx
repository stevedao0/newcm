      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          ...
          const contract = {
            id: `contract-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
            stt: parseInt(row.STT) || i + 1,
            linhVuc: row['Lĩnh vực'] || 'SCTT',
            ngayKy: formatDate(row['Ngày ký'] || ''),
            ...
            tinhTrang: row['Tình trạng'] || 'Đã ký',
            ...
            ngayBatDau: formatDate(row['Ngày bắt đầu'] || ''),
            ngayKetThuc: formatDate(row['Ngày hết hạn'] || ''),
            thoiGian: row['Thời gian'] || '',
            thoiLuong: row['Thời lượng'] || '',
            hinhThuc: row['Hình thức'] || '',
            mucNhuanBut: String(row['Mức nhuận bút'] || '0'),
